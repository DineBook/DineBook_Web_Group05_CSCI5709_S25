...
export class RestaurantsComponent implements OnInit {
    ...
    this.apiService.toggleFavorite(restaurant._id).subscribe({
      next: (res: any) => {
        ...
        if (res && typeof res.isFavorited === 'boolean') {
          ...
          console.warn(
            'Unexpected response format from toggleFavorite API:',
            res
          );
        }
      },
  ...
  private loadFavoriteStatus(): void {
    console.log(
      'Loading favorite status for restaurants:',
      this.restaurants.length
    );
    // Load favorite status asynchronously for each restaurant
    this.restaurants.forEach((restaurant, index) => {
      console.log(
        `Checking favorite status for restaurant: ${restaurant.name} (${restaurant._id})`
      );
      this.apiService.checkFavoriteStatus(restaurant._id).subscribe({
        next: (res: any) => {
          ...
          if (res && typeof res.isFavorited === 'boolean') {
            ...
            console.log(
              `Updated restaurant ${restaurant.name} isFavorite to:`,
              res.isFavorited
            );
          } else {
            console.warn(
              `Invalid response format for ${restaurant.name}:`,
              res
            );
          }
        },
        error: (error) => {
          console.error(
            `Error checking favorite status for restaurant ${restaurant._id}:`,
            error
          );
